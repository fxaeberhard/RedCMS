...
class FileBlock extends Block {
	...
	function getLabel() {
		...
		else return end(explode('/', $this->filePath));								//We strip the file from its path for display
	}	
	function getLink(){
			...
			foreach ($_FILES as $fieldName => $file) {
				switch ($file['error']) {
					case 0:														// No error during upload, proceed with file move
						...
						if (!file_exists($fPath)) mkdir($fPath, 0777, true);	// If the target directory does not exist, we create it
...
class BackupManager extends TreeStructure {
	function render() {
		...
		if (isset($_REQUEST['parentId'])) {
			...
			if (!file_exists($redCMS->fullpath.$fileDir)) mkdir($redCMS->fullpath.$fileDir, 0777, true);
