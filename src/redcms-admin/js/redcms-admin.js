    ...
    ContextMenu = Y.Base.create("contextmenu", Y.Widget, [Y.WidgetPosition, Y.WidgetStack, Y.WidgetChild], {
		...
		_onContextMenuItemSuccess: function(targetBlock, targetAdminNode) {
			...
			if (!targetAdminWidget) {														//HACK
				var targetAdmin = targetBlock.ancestor(function(n) {
						return (n.getAttribute('redadmin') != '')
					}, true);
		...
		_getMenuMarkupFromAdminNodesList : function( targetNode, targetAdminNodesList ) {
			var ret = new Array('<div class="yui3-menu-content">'),
				firstOfType = false,
				row, currentAdminNode;
				
				this._adminNodes = new Array();							//HACK
			
				for (var i=targetAdminNodesList.size()-1; i>=0; i--) {
					currentAdminNode = targetAdminNodesList.item(i);
					try {
						var jsonConf = Y.JSON.parse(decodeURI(currentAdminNode.getAttribute('redadmin')));
					} catch(e) {
		...
		_getMenuMarkupFromAdminNode : function( targetNode, targetAdminNode, adminConfObj ) {
			...
			for (var i=0; i<adminConfObj.length; i++) {
				row = adminConfObj[i];
				var isValidRow = true;
				if (row.filter != undefined) {
					var filterClasses = row.filter.split('|');
					isValidRow = filterClasses.indexOf(targetNode.getAttribute('widget')) > -1;
				}
				if (isValidRow) {
					var href = row.href,
						params = new Object(),
						//parentTuple = this._targetBlock.ancestor('[redid]'),
						parentNode = targetNode.ancestor( function(e) {
